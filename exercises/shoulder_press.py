...
class ShoulderPress(Exercise):
    def __init__(self):
        ...
        # Thresholds
        self.max_angle = 160  # Up position
        self.min_angle = 70   # Down position
    ...
    def process_landmarks(self, landmarks):
        ...
        # Counter Logic
        if active_angle < self.min_angle:
            self.stage = "down"
        if active_angle > self.max_angle and self.stage == 'down':
            self.stage = "up"
            self.counter += 1

        # Gauge Logic (70 -> 0%, 160 -> 100%)
        per = np.interp(active_angle, (self.min_angle, self.max_angle), (0, 100))
        bar = np.interp(active_angle, (self.min_angle, self.max_angle), (400, 150))
